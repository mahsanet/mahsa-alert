...
export const checkLocationProximity = (
  ...
  console.log('🎯 checkLocationProximity called with:', { userLocation, radiusKm });
  ...
  if (locationData.strikes?.features) {
    locationData.strikes.features.forEach((feature: any) => {
  ...
  if (locationData.sites?.features) {
    locationData.sites.features.forEach((feature: any) => {
      if (feature.geometry?.type === 'Point') {
        const point = turf.point(feature.geometry.coordinates);
        const distance = turf.distance(userPoint, point, 'kilometers');
  ...
  if (locationData.nuclear?.features) {
    locationData.nuclear.features.forEach((feature: any) => {
      if (feature.geometry?.type === 'Point') {
        const point = turf.point(feature.geometry.coordinates);
        const distance = turf.distance(userPoint, point, 'kilometers');
...
export const getDangerMessage = (pointType: 'strikes' | 'sites' | 'nuclear'): string => {
  switch (pointType) {
    case 'strikes':
