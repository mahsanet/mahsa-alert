...
import { checkLocationProximity, generateWarningMessage, type ProximityResult, type LocationData } from '../utils/locationProximity';
...
const ProximityAlert: React.FC<ProximityAlertProps> = ({ userLocation, locationData, isDarkMode }) => {
    ...
    <>
      ...
      <div className="fixed inset-4 z-50 flex items-center justify-center pointer-events-none">
        ...
        `}>
                ...
                {/* Header */}
        <div className="flex items-center justify-between p-3 sm:p-4 cursor-pointer border-b border-gray-200 dark:border-gray-700" onClick={handleMinimize}>
          <div className="flex items-center gap-3">
            <div className={`p-2 rounded-full ${
              ...
              <AlertTriangle 
                className={`w-5 h-5 ${
                  proximityResult.isInDanger ? 'text-orange-600 dark:text-orange-400' : 'text-green-600 dark:text-green-400'
                }`} 
              />
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">
                {proximityResult.isInDanger ? 'کلیک برای جزئیات' : 'شما در امان هستید'}
              </p>
            </div>
          </div>
        ...
        {!isMinimized && (
          <div className="p-4 sm:p-5 space-y-4">
            ...
            {proximityResult.isInDanger && proximityResult.nearbyPoints.length > 0 && (
              <div className="space-y-3">
                ...
                <div className="space-y-2 max-h-32 overflow-y-auto custom-scrollbar">
                  {proximityResult.nearbyPoints.slice(0, 5).map((point, index) => (
                    <div 
                      key={index} 
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        <span className="text-lg flex-shrink-0">
                          {point.type === 'strikes' ? '🎯' : point.type === 'nuclear' ? '☢️' : '🚀'}
                        </span>
                        <span className="text-xs sm:text-sm text-orange-800 dark:text-orange-200 truncate" dir="rtl">
                          {point.properties?.SiteTargeted || point.properties?.Site || point.properties?.BASES_CLUSTER || 'نقطه نامشخص'}
                        </span>
                      </div>
                  ...
                  {proximityResult.nearbyPoints.length > 5 && (
                    <div className="text-center py-2">
                      <span className="text-xs text-orange-600 dark:text-orange-400 bg-orange-100 dark:bg-orange-900/50 px-3 py-1 rounded-full">
                        و {proximityResult.nearbyPoints.length - 5} نقطه دیگر...
                      </span>
                    </div>
                  )}
                </div>
